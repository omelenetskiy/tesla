...
--
-- The Fleet path introduces two request kinds the Fleet API never had: a vehicle command
...
-- has to reflect both accepted shapes or the next reader will "fix" the wrong thing.
comment on column public.vehicles.provider_vehicle_id is 'Natural key from the provider: VIN for Fleet-based rows, short id for older rows.';
comment on column public.vehicles.vehicle_tag_id is 'Short id — valid as {vehicle_tag} alongside the VIN; never the long vehicle_id.';
