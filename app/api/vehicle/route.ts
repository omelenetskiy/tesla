...
import { FleetConfigError } from '@/lib/fleet/config'

export const dynamic = 'force-dynamic'

function serviceErrorMessage(error: unknown): string {
  if (error instanceof FleetConfigError) return `Fleet API is not configured: ${error.message}`
  if (error instanceof Error && /Supabase is not configured/i.test(error.message)) {
    return 'Server database credentials are not configured. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in Netlify environment variables.'
  }
  return error instanceof Error ? error.message : 'Service is temporarily unavailable.'
}
...
export async function GET(request: Request) {
  try {
    ...
    let rows = await listVehicleRows(user.id).catch(() => [])
    let row = pickFleetRow(rows, requestedVehicleId)
    if (!row || !fleetVehicleTag(row)) {
      ...
      rows = await listVehicleRows(user.id).catch(() => [])
      row = pickFleetRow(rows, requestedVehicleId)
    }
